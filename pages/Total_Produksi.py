...
st.title('Peramalan Produksi Total Produksi Pupuk')
...
if uploaded_file is not None:
    ...
    st.subheader("Menampilkan Sejumlah Dataset")
    st.write(df.head(5))
    ...
    st.subheader("Grafik Total Produksi Pupuk")
    
    #Membuat Grafik
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['Waktu'], y=df['total_produksi'], mode='lines', name='Total Produksi'))
    fig.update_layout(
        title="Total Produksi Pupuk di PT.PIM",
        xaxis_title='Waktu',
    ...
    z_scores = np.abs((df['total_produksi'] - df['total_produksi'].mean()) / df['total_produksi'].std())
    outliers = z_scores > 3
    mask = ~outliers
    df_interpolated = df.copy()
    df_interpolated.loc[outliers, 'total_produksi'] = np.interp(df.index[outliers], df.index[mask], df['total_produksi'][mask])
    df['total_produksi'] = df_interpolated['total_produksi']
    st.write(df.head(10))
    
    # Ekstraksi Fitur
    Average = (df['total_produksi']) / 3
    # Menambahkan fitur ke dataframe
    dataset = df.assign(Average=Average).fillna(0)
    
    dataset2 = dataset.sort_values(by=['Waktu']).copy()
    fitur = ['total_produksi', 'Average']
    ...
    train = dataset3['total_produksi'][:n_datalatih + 1]
    test = dataset3['total_produksi'][n_datalatih:]
    ...
    ax.set_title('Perbandingan Hasil Peramalan Data Latih dengan Data Uji (Total Produksi)')
    ax.set_xlabel('Waktu')
    ax.set_ylabel('Value')
    # Legenda
    ax.legend()
    ...
    st.subheader("Perhitungan WMA Untuk Seluruh Data Total Produksi Pupuk")
    ...
    def calculate_wma(data, weights):
        ...
        return wma
    total_produksi = df['total_produksi'].values
    # Melakukan perhitungan WMA
    wma_values = calculate_wma(total_produksi, weights)
    # Ambil data produksi amonia
    total_produksi = df['total_produksi'].values
    # Hitung WMA
    wma_values = calculate_wma(total_produksi, weights)
    # Membuat DataFrame dengan data asli dan nilai WMA
    df_wma = pd.DataFrame({'Total Produksi': x, 'wma_total_produksi': y} for x, y in zip(total_produksi, wma_values[:]))
    st.write(df_wma)
    ...
    mape = (np.abs(df_wma['Total Produksi'] - df_wma['wma_total_produksi']) / df_wma['Total Produksi']) * 100
    ...
    mad = (np.abs(df_wma['Total Produksi'] - df_wma['wma_total_produksi']))
    # mengubah nilai NaN dan inf menjadi 0
    ...
    tabel_akurasi = pd.DataFrame({
        'total_produksi': dataset3['total_produksi'],
        'wma_total_produksi': df_wma['wma_total_produksi'],
        'MAPE': mape,
        'MAD': mad
    })
    ...
    forecasts12 = forecast_wma(df_wma['Total Produksi'], weights)
    #Mengubah hasil peramalan dalam bentuk DataFrame
    forecasts12_df = pd.DataFrame({'Peramalan': forecasts12})
    ...
    fig.add_trace(go.Scatter(x=df['Waktu'], y=df['total_produksi'], mode='lines', name='Original', line=dict(color='blue')))
    fig.add_trace(go.Scatter(x=forecasts12_df['waktu'], y=forecasts12_df['Peramalan'], mode='lines', name='Forecast', line=dict(color='orange')))
    # Perbarui tata letak
    fig.update_layout(title='Grafik Peramalan Total Produksi Pupuk 12 bulan',
