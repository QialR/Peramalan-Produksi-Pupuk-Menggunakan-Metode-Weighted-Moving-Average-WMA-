...
st.title('Peramalan Produksi Pupuk Amonia')
...
if uploaded_file is not None:
    ...
    st.subheader("Grafik Produksi Amonia")
    fig = go.Figure()
    ...
    st.write(f'Total rows in training data: {train_data.shape[0]}')
    st.write(f'Total rows in testing data: {test_data.shape[0]}')
    ...
    fig.update_layout(title='Total Produksi Pupuk \n80:20 Training & Testing', xaxis_title='Total Data', yaxis_title='Total Produksi')
    st.plotly_chart(fig)
    ...
    ax.set_title('Perbandingan WMA Train dengan Data Test (Produksi Amonia)')
    ax.set_xlabel('Waktu')
    ax.set_ylabel('Value')
    # Legenda
    ax.legend()
    ...
    st.subheader("Perhitungan WMA Untuk Seluruh Data Produksi Amonia")
    # Pemberian bobot
    weights = [0.7, 0.2, 0.1]
    ...
    def calculate_wma(data, weights):
        ...
        return wma
    produks_amonia = df['produksi'].values
    # Melakukan perhitungan WMA
    wma_values = calculate_wma(produks_amonia, weights)
    ...
    df_wma = pd.DataFrame({'Produksi Amonia': x, 'wma_produksi_amonia': y} for x, y in zip(produks_amonia, wma_values[:]))
    st.write(df_wma)
    ...
    mape = (np.abs(df_wma['Produksi Amonia'] - df_wma['wma_produksi_amonia']) / df_wma['Produksi Amonia']) * 100
    ...
    mad = np.abs(df_wma['Produksi Amonia'] - df_wma['wma_produksi_amonia'])
    # mengubah nilai NaN dan inf menjadi 0
    ...
    st.subheader("Tabel Hasil Perhitungan WMA Produksi Amonia")
    tabel_akurasi = pd.DataFrame({
        'Produksi Amonia': dataset3['produksi'],
        'WMA Produksi Amonia': df_wma['wma_produksi_amonia'],
        'MAPE': mape,
        'MAD': mad
    })
    ...
    forecasts12 = forecast_wma(df_wma['Produksi Amonia'], weights)
    ...
    # Perbarui tata letak
    fig.update_layout(title='Grafik Peramalan Produksi Amonia 12 bulan',
